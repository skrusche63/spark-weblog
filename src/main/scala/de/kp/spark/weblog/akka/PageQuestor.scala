...
import de.kp.spark.weblog.{InsightResponse,PageInsightRequest}

import java.util.{Date,UUID}

class PageQuestor extends Actor with SparkActor {
  /*
   ...
   */
  private val sc = createLocalCtx("PageQuestor",props)
  ...
  def receive = {
    ...
    case req:PageInsightRequest => {
      ...
      val rows = LogInsight.fromPages(sc, path, query)
