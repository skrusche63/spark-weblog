...
class W3PageMiner(@transient sc:SparkContext) extends BaseActor {
  ...
  private def train(req:ServiceRequest) {
    ...
    /*
     * Compute time spent on each page within a session
     * and rate these pages with respect to the time spent;
     * the result is stored on the file system
     */
    val dataset = new W3LogEvaluator().logPages(sessions)    
    val sink = new W3LogSink(sc).saveLogPages(req, dataset)
         
    /* Update cache */
    cache.addStatus(req,ResponseStatus.MODEL_TRAINING_FINISHED)
    
    /* Notify potential listeners */
    notify(req,ResponseStatus.MODEL_TRAINING_FINISHED)
