...
class W3LogQuestor(@transient sc:SparkContext) extends BaseActor {
  ...
  def receive = {
    ...
    case req:ServiceRequest => {
      ...
      val response = try {
        ...
        retrieve(req).mapTo[ServiceResponse]
  ...
  private def retrieve(req:ServiceRequest):Future[Any] = {
  ...
  private def actor(req:ServiceRequest):ActorRef = {
    ...
    topic match {
      ...
      case "click" => context.actorOf(Props(new W3ClickQuestor(sc)))
      
      case "flow" => context.actorOf(Props(new W3FlowQuestor(sc)))
      case "page" => context.actorOf(Props(new W3PageQuestor(sc)))
