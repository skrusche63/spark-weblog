...
class W3PageQuestor(@transient sc:SparkContext) extends BaseActor {
  ...
  def receive = {
    ...
    case req:ServiceRequest => {
      ...
      val response = try {
        ...
        val flowfile = (Configuration.output(0) + "/pages/" + req.data(Names.REQ_UID))
        val query = req.data(Names.REQ_QUERY)
        
        val questor = new W3LogQueries(sc)
        val pages = LogPages(questor.fromPages(flowfile, query))
        
        val data = Map(Names.REQ_UID -> req.data(Names.REQ_UID),Names.REQ_RESPONSE -> Serializer.serializeLogPages(pages))
